...
int enE = 2, in1E = 3, in2E = 4, enD = 5, in3D = 6, in4D = 7; // ponte-h motores 
...
void loop()
{     
  // A9 sensorINFRA garra ---> 51 SensorToqueCima ---> 50 SensorToqueGarra 
   /*Serial.print("Frente-----> ");
 ...
 if((e == 1 && m == 1 && f == 1) || (m == 1 && f == 1 && d == 1) || (e == 1 && m == 1 && d == 1)){
...
void sala3(){
       ...
       while(ultraLateralE() < 120){
         oeste(210); //
       }
       delay(300);
       norte(0); 
       delay(200); //
       while(ultraFrente() >= 40){
         frente();
       }
       while(ultraFrente() <= 45){
        oeste(210); //
        if(ultraFrente() <= 29){
          if(ultraFrente() < 25){
            sul(80);
          ...
          if(analogRead(A9) > 600){
            ...
            norte(0);
            delay(500);
            motorFEC.write(8); //maximo 50 ( para varrer) minimo 15 (fechar) andar (120)
            delay(1000);
            motorELE.write(50); //alto 40  baixo 140 alto
            delay(1000);
...
void entregaD () {
  int b=0;
  while ( b ==0){ 
    while (digitalRead(11)==0 && digitalRead(26)==0){ // verificarigit porta
      norte(90);
    }
    if (digitalRead(11)==1){
      oeste(200);
    ...
    if (digitalRead(26)==1 && digitalRead(11)==0){
      motorELE.write(110); //alto 40  baixo 140 alto
      delay(1000);
      motorFEC.write(60); //maximo 50 ( para varrer) minimo 15 (fechar) andar (120)
      delay(1000);
    }
    voltarProcurarD();
  }  
...
void voltarProcurarD(){
    ...
    while (u == 0) {
      ...
      else if (ultraLateralE()>30){
        oeste(200);
        delay(400);
      }
      while(ultraFrente() >= 50){
         frente();
      }
      while(ultraFrente() >= 40){
      oeste(170); //
      ...
      if(ultraFrente() <= 40){
         ...
         delay(700);
         motorELE.write(135); //alto 40  baixo 140 alto
         motorFEC.write(85); //maximo 50 ( para varrer) minimo 15 (fechar) andar (120)
         norte(0);
         delay(400);
         while(analogRead(A9) < 600){
           frente();
           if(analogRead(A9) > 600){
             ...
             while (f == 0){  
              ...
              norte(0);
              delay(500);
              motorFEC.write(15); //maximo 50 ( para varrer) minimo 15 (fechar) andar (120)
              delay(1000);
              motorELE.write(50); //alto 40  baixo 140 alto
              delay(3000);
              entregaD();
